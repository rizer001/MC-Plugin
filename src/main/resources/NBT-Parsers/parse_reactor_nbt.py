...
f = gzip.open('src/main/resources/NBT-Files/reactorcore1.nbt', 'rb')
d = f.read()
f.close()
...
print('=' * 60)
print('  STRUCTURE: reactorcore1.nbt')
print('=' * 60)
...
print('-' * 60)
print('  TOP-CENTER BLOCK (frame attachment point)')
print('-' * 60)
print(f'  NBT center = ({sx // 2}, {sy - 1}, {sz // 2})')
tcx, tcy, tcz = sx // 2, sy - 1, sz // 2
tc_state = None
for block in blocks:
    pos = block.get('pos')
    if pos and pos[0] == tcx and pos[1] == tcy and pos[2] == tcz:
        tc_state = block.get('state')
        break
if tc_state is not None and tc_state < len(short_names):
    print(f'  Block at top-center: {short_names[tc_state][1]}')
else:
    print(f'  Block at top-center: AIR (empty)')
print(f'  Frame goes on TOP FACE of this block')
print(f'  Frame entity position: ({tcx}, {tcy + 1}, {tcz})')
