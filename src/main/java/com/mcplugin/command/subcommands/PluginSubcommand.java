...
public final class PluginSubcommand {
    ...
    private static boolean handlePluginAction(CommandSender sender, String[] args) {
        ...
        // Info — read-only, доступно для любых плагинов включая MC-Plugin
        ...
        if (target.getName().equals("MC-Plugin")) {
            sender.sendMessage(MessageUtil.parse(
                    "<dark_red>❌</dark_red> <red>Cannot manage MC-Plugin itself. Use </red><white>/mp reload</white><red> instead.</red>"));
            return true;
        }
