...
public abstract class AbstractCheck implements Listener {
    ...
    public void loadConfig() {
        var cfg = com.ultimateimprovements.core.Main.getInstance().getConfig();
    ...
    protected String getConfigString(String key, String def) {
        return com.ultimateimprovements.core.Main.getInstance().getConfig().getString(configPath + "." + key, def);
    }

    protected boolean getConfigBoolean(String key, boolean def) {
        return com.ultimateimprovements.core.Main.getInstance().getConfig().getBoolean(configPath + "." + key, def);
    }

    protected double getConfigDouble(String key, double def) {
        return com.ultimateimprovements.core.Main.getInstance().getConfig().getDouble(configPath + "." + key, def);
    }

    protected int getConfigInt(String key, int def) {
        return com.ultimateimprovements.core.Main.getInstance().getConfig().getInt(configPath + "." + key, def);
    }
