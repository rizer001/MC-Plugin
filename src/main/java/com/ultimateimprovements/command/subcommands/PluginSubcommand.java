...
public final class PluginSubcommand {
    ...
    private static boolean handlePluginAction(CommandSender sender, String[] args) {
        ...
        // Info — read-only, доступно для любых плагинов включая UltimateImprovements
        ...
        if (target.getName().equals("UltimateImprovements")) {
            sender.sendMessage(MessageUtil.parse(
                    "<dark_red>❌</dark_red> <red>Cannot manage UltimateImprovements itself. Use </red><white>/mp reload</white><red> instead.</red>"));
            return true;
        }
